...
function logTransaction(sender, receiver, amount, type) {
    ...
    if (receiver === currentUser.username) {
        // Ajouter le log uniquement si le receiver est l'utilisateur courant
...
function addLogToSection(logText, receiver) {
    ...
    if (currentUser && currentUser.username === receiver) {
...
function logTransaction(sender, receiver, amount, type) {
    ...
    // Ajouter un log de transaction à la section des logs pour le côté du sender
    addLogToSection(`${sender} a effectué une transaction de ${amount} € vers ${receiver} le ${logEntry.date}`, receiver);

    // Ajouter un log de transaction à la section des logs pour le côté du receiver
    addLogToSection(`${receiver} a reçu une transaction de ${amount} € de ${sender} le ${logEntry.date}`, receiver);
}
